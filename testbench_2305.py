        ...
        for n_np in n_next_patches:
            for reduction_factor in reduction_factors:
                ...
                for pooling_strategy in pooling_strategies:
                    ...
                    model.group_id = this_run_id + '-' + str(pooling_strategy) + '-reduced_by_' + str(reduction_factor) + '-RN34-L_2_normalized-entropy_normed-' + str(n_n) + 'NN_' + str(n_np) + 'NP (adapted)'
                    run(model, False)
    model.adapted_score_calc = False
    for n_n in n_neighbors:
        for n_np in n_next_patches:
            for reduction_factor in reduction_factors:
                ...
                for pooling_strategy in pooling_strategies:
                    model.pooling_strategy = pooling_strategy
                    model.group_id = this_run_id + '-' + str(pooling_strategy) + '-reduced_by_' + str(reduction_factor) + '-RN34-L_2_normalized-entropy_normed-' + str(n_n) + 'NN ' + '(not adapted)'
                    run(model, False)                 
    # model.reduce_via_entropy_normed = False
