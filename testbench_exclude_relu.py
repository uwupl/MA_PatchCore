...
if __name__ == '__main__':
    ...
    model.coreset_sampling_ratio = 0.01 #100%
    ...
    model.exclude_relu = False 
    model.model_id = 'RN18'
    model.group_id = this_run_id + 'with_ReLu_RN18'
    if not os.path.exists(os.path.join(res_path, model.group_id)):
        try:
            print('Run ', run_counter+1, ' of ', total_runs, ' started.')
            run(model, True)
    ...
    model.exclude_relu = True
    model.model_id = 'RN18'
    model.group_id = this_run_id + 'without_ReLu_RN18'
    if not os.path.exists(os.path.join(res_path, model.group_id)):
        try:
            print('Run ', run_counter+1, ' of ', total_runs, ' started.')
            run(model, True)
    ...
    model.sigmoid_in_last_layer = True 
    model.model_id = 'RN18'
    model.group_id = this_run_id + 'with_Sigmoid_RN18'
    if not os.path.exists(os.path.join(res_path, model.group_id)):
        try:
            print('Run ', run_counter+1, ' of ', total_runs, ' started.')
            run(model, True)
